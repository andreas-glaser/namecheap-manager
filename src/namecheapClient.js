...
class NamecheapClient {
  constructor() {
    for (const k of ['apiUser', 'apiKey', 'userName', 'clientIp']) {
      if (!cfg[k]) throw new Error(`Missing config value ${k} – check .env or config.json`);
    }
  }
  ...
  async enablePrivacy(domain) {
    ...
    const e = list.find(x => x.domainName === domain);
    if (!e) {
      throw new Error(`No privacy subscription exists for ${domain}. Purchase via dashboard first.`);
    }
    if (e.isEnabled) return;
    await this.#requestNoDomain('namecheap.whoisguard.enable', { WhoisguardID: e.whoisguardId });
  }
  ...
  async disablePrivacy(domain) {
    ...
    if (!e.isEnabled) return;
    await this.#requestNoDomain('namecheap.whoisguard.disable', { WhoisguardID: e.whoisguardId });
  }
